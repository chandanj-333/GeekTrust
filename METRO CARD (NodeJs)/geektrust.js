...
const summary = require('./summary.json') //Used to store the analytics of a station

const cards = {} //cards object contains card and balance available
/*
Sample cards= { MC1: 250, MC2: 50, MC3: 0, MC4: 50 }
*/

const originStations = {} //originStations contains origin station of journey of cards
/*
Sample originStations= { MC2: 'CENTRAL', MC3: 'AIRPORT' }
*/
...
function checkCardBalance(card,source,requiredAmount){
    ...
    if(cards[card]<requiredAmount) {
        ...
        summary[source].total_collection+=interest
...
function checkIn(card,passengerType,source) {
    ...
    if(Object.keys(cards).includes(card)) {  //Continue with the execution if card exists
        if(originStations[card]===source) {
            //if passenger returns to the origin station,apply discount of 50% (prices.discount)
            const discount = prices[passengerType]*prices.discount
            checkCardBalance(card,source,discount)
            cards[card]-=discount
            summary[source].total_collection+=prices[passengerType]-discount
            ...
            /*if the station is not his origin station,checkin with actual prices and 
                add the origin station if it is his first station */
            checkCardBalance(card,source,prices[passengerType])
            cards[card]-=prices[passengerType]
            if(!originStations[card])
                originStations[card] = source ==='CENTRAL'?'AIRPORT':'CENTRAL'
            summary[source].total_collection+=prices[passengerType]
            summary[source]['counts'][passengerType]+=1
        }
    }
}
