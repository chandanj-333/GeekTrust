...
function checkCardBalance(card, source, requiredAmount) {
	...
	if (cards[card] < requiredAmount) {
		const interest =
			(requiredAmount - cards[card]) * prices.walletTopUpInterest;
...
function checkIn(card, passengerType, source) {
	...
	if (Object.keys(cards).includes(card)) {
		//Continue with the execution if card exists
		if (originStations[card] === source) {
			...
			if (!originStations[card])
				//Add origin station if it is his first station
